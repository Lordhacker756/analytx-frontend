...
export function RateIntern() {
    ...
    return (
        <div className="flex flex-col min-h-screen w-full dark:bg-gray-900 md:flex-row">
            <div
                className="w-full border-b bg-gray-800 p-6 dark:bg-gray-800 md:border-r md:w-[300px] md:border-b-0 md:p-6">
                <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-gray-50">Form Templates</h3>
                    <Select
                        onValueChange={(v) => {
                            console.log(v)
                            setSelectedForm(v)
                    ...
                    >
                        <SelectTrigger className="w-full">
                            <SelectValue className="text-gray-50" placeholder="Select a form"/>
                        </SelectTrigger>
            ...
            {
                selectedForm == 1 ? <SearchEngineQuestionnare/> :
                    selectedForm == 2 ? <TeachATopic/> :
                        selectedForm == 3 ? <GroupDiscussion/> :
                            selectedForm == 4 ? <GoogleSpreadsheet/> :
                                selectedForm == 5 ? <Origami/> :
                                    selectedForm == 6 ? <TeamTask/> :
                                        selectedForm == 7 ? <Punctuality/> :
                                            selectedForm == 8 ? <Attendance/> :
                                                selectedForm == 9 ? <PersonalityType/> : <PersonalSuggestion/>
            }
        </div>
    )
}